...
export default async function handler(req: Req, res: Res) {
  ...
  if (req.method === 'GET') {
    ...
    if (id) {
      const { data, error } = await auth.supabase.from('leads').select('*').eq('id', id).single();
  ...
  if (req.method === 'PATCH') {
    ...
    const { data, error } = await auth.supabase.from('leads').update(updates).eq('id', body.id).select().single();
